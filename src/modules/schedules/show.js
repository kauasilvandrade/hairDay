import dayjs from "dayjs";

const perioMorning = document.getElementById("period-morning")
const perioAfternoon = document.getElementById("period-afternoon")
const perioNight = document.getElementById("period-night")

export function scheduleShow({dailySchedules}) {
  try {
    perioMorning.innerHTML = ""
    perioAfternoon.innerHTML = ""
    perioNight.innerHTML = ""

    dailySchedules.forEach(schedule => {
      const item = document.createElement("li")
      const time = document.createElement("strong")
      const name = document.createElement("span")

      item.setAttribute("data-id", schedule.id)

      time.textContent = dayjs(schedule.when).format("HH:mm")
      name.textContent = schedule.name

      const cancelIcon = document.createElement("img")
      cancelIcon.classList.add("cancel-icon")
      cancelIcon.setAttribute("src", "./src/assets/cancel.svg")
      cancelIcon.setAttribute("alt", "Cancelar")

      item.append(time, name, cancelIcon)

      const hour = dayjs(schedule.when).hour()

      if (hour<=12) {
        perioMorning.appendChild(item)
      } else if (hour>12 && hour <= 18) {
        perioAfternoon.appendChild(item)
      } else {
        perioNight.appendChild(item)
      }
    });
  } catch (error) {
   alert("Não foi possível exbir os agendamentos!") 
  }
}